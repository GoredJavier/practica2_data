...
```{r print_datatable, include=FALSE}

## (Alternativa al comando datable, pero es mas sencillo) -- knitr::kable(df)
datatable(df)
...
#ggtab <- ggtexttable(table_df, cols = colnames(table_df))
...
```{r}
ggplot(df,aes(tipo_url))+geom_bar(fill = "lightblue", color = "black")
```
...
ggplot(df,aes(url_relativa))+geom_bar(fill = "lightblue", color = "black")

length(which(grepl("Absoluta", df$tipo_url, fixed=FALSE)))
length(which(grepl("Relativa", df$tipo_url, fixed=FALSE)))

Posición_Url_Relativa <- which(grepl("Relativa", df$tipo_url, fixed=FALSE))
df$url_absoluta[prueba]

Posición_Url_Absoluta <- which(grepl("Absoluta", df$tipo_url, fixed=FALSE))
Url_ordenadas <- df$url_absoluta[Posición_Url_Absoluta]
